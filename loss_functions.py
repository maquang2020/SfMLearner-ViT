...
def one_scale_reconstruction(tgt_img, ref_imgs, intrinsics, depth, explainability_mask, pose, args):
    ...
    for i, ref_img in enumerate(ref_imgs_scaled):
        ...
        ref_img_warped, valid_points = inverse_warp(ref_img, depth_scaled[:,0], current_pose,
                                                    intrinsics_scaled,
...
def photometric_reconstruction_loss(tgt_img, ref_imgs, intrinsics, depth, explainability_mask, pose, args):
    ...
    total_loss, _, _ = one_scale_reconstruction(tgt_img, ref_imgs, intrinsics, depth, explainability_mask, pose, args)
...
def photometric_reconstruction_results(tgt_img, ref_imgs, intrinsics, depth, explainability_mask, pose, args):
    ...
    _, warped, diff = one_scale_reconstruction(tgt_img, ref_imgs, intrinsics, depth, explainability_mask, pose, args)
...
def edge_aware_smoothness_loss(img, pred_disp):
    ...
    b, _, h, w = img.size()
    scaled_pred_disp = nn.functional.adaptive_avg_pool2d(pred_disp, (h, w))
    loss += get_edge_smoothness(img, scaled_pred_disp)
    weight /= 2.3   # 2sqrt(2)
